---
title: Py项目开发实践经验——Py通过 Nuitca 转译成 C 再打包成可执行文件
...
![可以看到无限请求服务器](./277150786.jpg)

调试环境时的输出：

![同样的代码不一样的逻辑](./4055214554.png)

这可不是我们代码层面的问题啊，**我反复调试多次过编译，也把相关代码单独拿来调试过，可单独代码段运行正常，而且我的电脑还会发生一种奇怪的现象**：

![《幽默》](./824548251.png)

_**我的 CPU 被吃满啦！ o(≧口≦)o**_
...
1. **用于执行Nuitka的Python解释器最好是CPython，即Python解释器的标准实现。**（conda、官方python均是）使用Apple Python等Python解释器部分功能将受限。
